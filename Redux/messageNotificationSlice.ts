...
const messageNotificationSlice = createSlice({ name: 'messageNotification', initialState, reducers: { setMessageNotification: ( state, action: PayloadAction<{ message: string; email: string }> ) => { state.message = action.payload.message; state.email = action.payload.email; localStorage.setItem(localStorageKey, JSON.stringify(state)); }, clearMessageNotification: (state) => { state.message = ''; state.email = ''; localStorage.removeItem(localStorageKey); }, }, });
