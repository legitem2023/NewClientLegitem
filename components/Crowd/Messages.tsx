...
useEffect(() => { const unsubscribe = subscribeToMore({ document: MESSAGE_ADDED, updateQuery: (prev, { subscriptionData }) => { 
...
return ( <ReusableCenterLayout 
            child1={() => <></>} 
            child2={() => activeStream ? ( <div className="messagesLI"> <video ref={videoRef} className="messageVideo" autoPlay playsInline muted /> </div> ) : null } 
            child3={() => ( 
                  <div style={{ minHeight: '92vh', height: 'auto', width: '100%' }}> 
                        ...
                        <AutoSizer> {({ height, width }) => ( isListLoading ? ( <CrowdLoading /> ) : ( 
                        <List height={height} width={width} rowHeight={cache.current.rowHeight} deferredMeasurementCache={cache.current} rowCount={result.length} ref={listRef} rowRenderer={({ key, index, style, parent }) => ( 
<CellMeasurer
key={key}
cache={cache.current}
columnIndex={0}
...
parent={parent}
> {({ measure }) => ( 
      <div style={{ ...style, marginBottom: '15px', padding: '10px 0', display: 'flex', flexDirection: 'column', width: '100%', scrollSnapType: 'both mandatory', }} onLoad={measure} > {isListLoading ? ( 
            <div className="skeleton-loader" style={{ height: '50px', background: '#f0f0f0' }} /> ) : ( 
            <ReusableMessage data={result[index]} onChange={measure} /> )} </div> )} 
</CellMeasurer> )} /> ) )} 
</AutoSizer> 
</div> )} 
child4={() => <></>} /> ); };
